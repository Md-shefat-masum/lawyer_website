                    ...
                    <div class="row">
                        ...
                        <div class="col-6">
                            <a href="#" class="btn btn-outline-tumblr float-right add_btn"
                                data-href="{{ route('admin_review_store') }}" data-add_href="{{ route('admin_review_add') }}"
                                ...
                                <tbody>
                                    ...
                                    @foreach ($reviews as $key => $item)
                                        <tr>
                                            <th scope="row">{{ $i++ }}</th>
                                            <td>{{ $item->name }}</td>
                                            <td style="white-space: break-spaces;word-break: break-word;">{{ $item->description }}</td>
                                            <td>{{ $item->star }}</td>
                                            <td>{{ $item->created_at->format('d-M-Y h:i:s a') }}</td>
                                            <td>
                                                <ul class="d-flex justify-content-center table_action_list">
                                                    <li><a class="data_view_btn" href="{{ route('admin_review_view',$item->id) }}"><i class="fa fa-plus"></i></a></li>
                                                    <li>
                                                        <a href="#" class="edit_btn"
                                                            data-href="{{ route('admin_review_update') }}"
                                                            data-edit_href="{{ route('admin_review_edit') }}?id={{ $item->id }}"
                                                            data-toggle="modal" data-target="#formemodal">
                                                            <i class="fa fa-edit"></i>
                                                        </a>
                                                    </li>
                                                    <li><a href="#" data-href="{{ route('admin_review_delete', $item->id) }}"
