                    ...
                    <div class="row">
                        ...
                        <div class="col-6">
                            {{-- <a href="#" class="btn btn-outline-tumblr float-right add_btn"
                                data-href="{{ route('admin_contact_us_message_store') }}" data-add_href="{{ route('admin_contact_us_message_add') }}"
                        ...
                        <div class="table-responsive">
                            <table class="table text-center">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($contact_us_messages as $key => $item)
                                        <tr>
                                            <th scope="row">{{ $i++ }}</th>
                                            <td>{{ $item->name }}</td>
                                            <td>{{ $item->email }}</td>
                                            <td>{{ $item->phone }}</td>
                                            <td>{{ $item->website }}</td>
                                            <td style="width: 400px;word-break: break-word;white-space: break-spaces; ">{{ Str::limit($item->message,20,'..') }}</td>
                                            <td>{{ $item->created_at->format('d-M-Y h:i:s a') }}</td>
                                            <td>
                                                <ul class="d-flex justify-content-center table_action_list">
                                                    <li><a class="data_view_btn" href="{{ route('admin_contact_us_message_view',$item->id) }}"><i class="fa fa-plus"></i></a></li>
                                                    {{-- <li>
                                                        <a href="#" class="edit_btn"
                                                            data-href="{{ route('admin_contact_us_message_update') }}"
                                                            data-edit_href="{{ route('admin_contact_us_message_edit') }}?id={{ $item->id }}"
                                                            data-toggle="modal" data-target="#formemodal">
                                                            <i class="fa fa-edit"></i>
                                                        </a>
                                                    </li> --}}
                                                    <li><a href="#" data-href="{{ route('admin_contact_us_message_delete', $item->id) }}"
